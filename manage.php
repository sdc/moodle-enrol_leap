...
$instance = $DB->get_record('enrol', array('id'=>$enrolid, 'enrol'=>'leap'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id'=>$instance->courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

require_login($course);
$canenrol = has_capability('enrol/leap:enrol', $context);
$canunenrol = has_capability('enrol/leap:unenrol', $context);
...
if (!$canenrol and !$canunenrol) {
    ...
    require_capability('enrol/leap:unenrol', $context);
}
...
if (!$enrol_leap = enrol_get_plugin('leap')) {
    throw new coding_exception('Can not instantiate enrol_leap');
}

$instancename = $enrol_leap->get_instance_name($instance);

$PAGE->set_url('/enrol/leap/manage.php', array('enrolid'=>$instance->id));
$PAGE->set_pagelayout('admin');
$PAGE->set_title($enrol_leap->get_instance_name($instance));
...
$potentialuserselector = new enrol_leap_potential_participant('addselect', $options);
$currentuserselector = new enrol_leap_current_participant('removeselect', $options);
...
if ($canenrol && optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        foreach($userstoassign as $adduser) {
            ...
            $enrol_leap->enrol_user($instance, $adduser->id, $roleid, $timestart, $timeend);
        }
...
if ($canunenrol && optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstounassign)) {
        foreach($userstounassign as $removeuser) {
            $enrol_leap->unenrol_user($instance, $removeuser->id);
        }
    ...
    <tr>
      ...
      <td id="buttonscell">
          <div id="addcontrols">
              ...
              <p><label for="menuroleid"><?php print_string('assignrole', 'enrol_leap') ?></label><br />
