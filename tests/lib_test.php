 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_leap_lib_testcase extends advanced_testcase {
    /**
    ...
    public function test_migrate_plugin_enrolments() {
        ...
        /** @var $manplugin enrol_leap_plugin */
        $manplugin = enrol_get_plugin('leap');
        ...
        // Enrol some users to leap instances.

        $maninstance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $DB->set_field('enrol', 'status', ENROL_INSTANCE_DISABLED, array('id'=>$maninstance1->id));
        $maninstance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $maninstance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $DB->delete_records('enrol', array('courseid'=>$course3->id, 'enrol'=>'leap'));
        $DB->delete_records('enrol', array('courseid'=>$course4->id, 'enrol'=>'leap'));
        $DB->delete_records('enrol', array('courseid'=>$course5->id, 'enrol'=>'leap'));
        ...
        enrol_leap_migrate_plugin_enrolments('xxx');

        // Verify results.

        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course1->id, 'enrol'=>'leap')));
        ...
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course2->id, 'enrol'=>'leap')));
        ...
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course3->id, 'enrol'=>'leap')));
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course3->id, 'enrol'=>'xxx')));
        $maninstance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        ...
        $this->assertEquals(1, $DB->count_records('enrol', array('courseid'=>$course4->id, 'enrol'=>'leap')));
        $this->assertEquals(2, $DB->count_records('enrol', array('courseid'=>$course4->id, 'enrol'=>'xxx')));
        $maninstance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        ...
        $this->assertEquals(0, $DB->count_records('enrol', array('courseid'=>$course5->id, 'enrol'=>'leap')));
        ...
        enrol_leap_migrate_plugin_enrolments('leap');
        enrol_leap_migrate_plugin_enrolments('yyyy');
    }
    ...
    public function test_expired() {
        ...
        /** @var $leapplugin enrol_leap_plugin */
        $leapplugin = enrol_get_plugin('leap');
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'leap')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance1->roleid);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance2->roleid);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance3->roleid);

        $this->assertEquals(0, $DB->count_records('user_enrolments'));
        $this->assertEquals(0, $DB->count_records('role_assignments'));

        $leapplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $leapplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $leapplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $leapplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $leapplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now+60*60);
        $leapplugin->enrol_user($instance3, $user3->id, $teacherrole->id, 0, $now-60*60);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $leapplugin->get_config('expiredaction'));
        $leapplugin->sync($trace, null);
        $this->assertEquals(6, $DB->count_records('user_enrolments'));
        $this->assertEquals(7, $DB->count_records('role_assignments'));


        $leapplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $leapplugin->sync($trace, $course2->id);
        $this->assertEquals(6, $DB->count_records('user_enrolments'));
        $this->assertEquals(7, $DB->count_records('role_assignments'));
        ...
        $leapplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
        ...
        $leapplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPEND);
        $leapplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);
        $leapplugin->enrol_user($instance3, $user3->id, $teacherrole->id, 0, $now-60*60);
        $maninstance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $maninstance2 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'leap'), '*', MUST_EXIST);
    ...
    public function test_send_expiry_notifications() {
        ...
        $leapplugin->set_config('expirynotifylast', $now - 60*60*24);
        $leapplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'leap')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'leap'), '*', MUST_EXIST);
        $instance4->expirythreshold = 60*60*24*1;
        $instance4->expirynotify    = 0;
        $instance4->notifyall       = 0;
        $DB->update_record('enrol', $instance4);

        $leapplugin->enrol_user($instance1, $user1->id, $editingteacherrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $leapplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $leapplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $leapplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $leapplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $leapplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        $leapplugin->enrol_user($instance1, $user7->id, $editingteacherrole->id);
        $leapplugin->enrol_user($instance1, $user8->id, $managerrole->id);                                             // Highest role --> enroller.

        $leapplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $leapplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $leapplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $leapplugin->enrol_user($instance3, $user1->id, $editingteacherrole->id);
        $leapplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $leapplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $leapplugin->enrol_user($instance4, $user4->id, $editingteacherrole->id);
        $leapplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $leapplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $leapplugin->send_expiry_notifications($trace);
        ...
        $leapplugin->set_config('expirynotifylast', time() - 60*60*24);
        $leapplugin->set_config('expirynotifyhour', '24');
