...
/**
 * Auxiliary Leap user enrolment lib, the main purpose is to lower memory requirements...
 *
 * @package    enrol_leap
 ...
 * Enrol candidates.
 */
class enrol_leap_potential_participant extends user_selector_base {
    protected $enrolid;
    ...
    protected function get_options() {
        ...
        $options['file']    = 'enrol/leap/locallib.php';
        return $options;
    }
}
 ...
 * Enrolled users.
 */
class enrol_leap_current_participant extends user_selector_base {
    protected $courseid;
    protected $enrolid;
...
/**
 * A bulk operation for the Leap enrolment plugin to edit selected users.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_leap_editselectedusers_operation extends enrol_bulk_enrolment_operation {
    ...
    public function get_title() {
        return get_string('editselectedusers', 'enrol_leap');
    }
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/leap:manage", $manager->get_context())) {
            return false;
        }
     ...
     * @return enrol_leap_editselectedusers_form
     */
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        global $CFG;
        require_once($CFG->dirroot.'/enrol/leap/bulkchangeforms.php');
        return new enrol_leap_editselectedusers_form($defaultaction, $defaultcustomdata);
    }
}


/**
 * A bulk operation for the Leap enrolment plugin to delete selected users enrolments.
 *
 * @copyright 2011 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_leap_deleteselectedusers_operation extends enrol_bulk_enrolment_operation {
     ...
     */
    public function get_title() {
        return get_string('deleteselectedusers', 'enrol_leap');
    }
    ...
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        ...
        $defaultcustomdata['message'] = get_string('confirmbulkdeleteenrolment', 'enrol_leap');
        $defaultcustomdata['button'] = get_string('unenrolusers', 'enrol_leap');
        return new enrol_leap_deleteselectedusers_form($defaultaction, $defaultcustomdata);
    }
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/leap:unenrol", $manager->get_context())) {
            return false;
        }
        foreach ($users as $user) {
            foreach ($user->enrolments as $enrolment) {
...
/**
 * Migrates all enrolments of the given plugin to enrol_leap plugin,
 ...
 */
function enrol_leap_migrate_plugin_enrolments($enrol) {
    global $DB;
    ...
    $sql = "SELECT e.id, e.courseid, e.status, MIN(me.id) AS mid, COUNT(ue.id) AS cu
         ...
         LEFT JOIN {enrol} me ON (me.courseid = e.courseid AND me.enrol='leap')
             WHERE e.enrol = :enrol
    ...
    foreach($rs as $e) {
        $minstance = false;
        if (!$e->mid) {
            // Leap instance does not exist yet, add a new one.
            $course = $DB->get_record('course', array('id'=>$e->courseid), '*', MUST_EXIST);
            if ($minstance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'leap'))) {
                // Already created by previous iteration.
                $e->mid = $minstance->id;
            } else if ($e->mid = $leapplugin->add_default_instance($course)) {
        ...
        if (!$minstance) {
            ...
            debugging('Failed to find leap enrolment instance', DEBUG_DEVELOPER);
            continue;
        }
        ...
        // Migrate to Leap enrol instance.
        $params = array('id'=>$e->id, 'mid'=>$e->mid);
 ...
 */
function enrol_leap_get_potential_cohorts($context, $enrolid, $search = '', $page = 0, $perpage = 25, $addedenrollment = 0) {
    global $CFG;
